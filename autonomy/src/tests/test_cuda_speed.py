...
def run_cuda_hough(image, iterations=50):
    ...
    hough_detector = cv2.cuda.createHoughLinesDetector(1, np.pi / 180, 80, 30, 10)
...
if __name__ == "__main__":
    ...
    print(f"CPU average iteration time: {cpu_time:.4f} seconds")
    print(f"GPU average iteration time: {gpu_time:.4f} seconds" if gpu_time is not None else "GPU iterations time: CUDA not available or failed")
