...
class VisualServo(Node):
    def __init__(self):
        ...
        self.camera_frame_id = "gripper_camera"
        ...
        self.max_kf_label = 0
        # wires are normally around more than 4.0 distance from eachother
        self.distance_threshold = 1.0
        self.line_length = None
        self.valid_threshold = 20
        self.update_freq = 0.1
        self.tracked_wire_id = None
        self.total_iterations = 0
        self.target_start_threshold = 30

        # Subscribers
        self.camera_info_sub = self.create_subscription(CameraInfo, '/robot_1/sensors/gripper_cam/camera_info', self.camera_info_callback, 10)
        self.rgb_image_sub = Subscriber(self, Image, '/robot_1/sensors/gripper_cam/image_rect')
        self.depth_image_sub = Subscriber(self, Image, '/robot_1/sensors/gripper_cam/depth')
        self.semantics_sub = Subscriber(self, Image, '/robot_1/sensors/gripper_cam/segmentation')
        ...
        self.tf_timer = self.create_timer(self.update_freq, self.timer_tf_callback, callback_group=concurrent_cb_group)

        # Service
        self.activate_srv = self.create_service(Trigger, '/robot_1/behavior/toggle_visual_servo_command', self.activate_callback)

        # Publishers
        self.velocity_pub = self.create_publisher(TwistStamped, "/robot_1/interface/mavros/setpoint_velocity/cmd_vel", 10)
        self.position_pub = self.create_publisher(PoseStamped, "/robot_1/interface/mavros/setpoint_position/local", 10)
        self.img_debug_pub = self.create_publisher(Image, "/robot_1/sensors/gripper_cam/detection_debug", 2)
        self.line_location_vis_pub = self.create_publisher(Marker, "/robot_1/visual_servo/line_location_visualization", 2)
    ...
    def detect_lines_and_update(self, image, depth, segmentation, timestamp):
        ...
        if np.any(seg_mask) and self.received_first_tf:
            wire_lines, wire_midpoints, avg_yaw = self.wire_detector.detect_wires_from_segmentation(seg_mask)
            try:
            ...
            if self.tracked_wire_id is None and self.total_iterations > self.target_start_threshold:
                ...
                for i, kf in self.position_kalman_filters.items():
                    if kf.valid_count >= self.valid_threshold:
                        curr_pos = self.tf_buffer.lookup_transform(self.world_frame_id, self.camera_frame_id, timestamp).transform.translation
            ...
            if self.are_kfs_initialized:
                debug_img = self.draw_valid_kfs(debug_img, transform)
            return debug_img
    ...
    def add_kf(self, midpoint):
        ...
        while saturation < saturation_threshold and value < value_threshold:
            ...
            value = hsv[0, 0, 2]
        self.get_logger().info(f'color: {color}, saturation: {saturation}, value: {value}')
